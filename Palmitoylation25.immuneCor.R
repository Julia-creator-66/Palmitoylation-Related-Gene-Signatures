...
immFile="immuneScore.txt"          #免疫细胞浸润的结果文件
setwd("C:\\Users\\ljz\\Desktop\\25.immuneCor")     #设置工作目录
...
#去除对照组样品
group=gsub("(.*)\\_(.*)\\_(.*)", "\\3", colnames(data))
data=data[,group=="Treat",drop=F]
data=t(data)

#读取免疫细胞的打分文件，并对数据进行整理
...
pdf(file="cor.pdf", width=7.5, height=4.5)
ggplot(outTab, aes(Immune, Gene)) + 
	...
	scale_x_discrete(position = "bottom")      #X轴免疫细胞名称显示的位置
dev.off()
